...
func (c *spanCollector) KeepGroup(res *parquetquery.IteratorResult) bool {
	...
	for _, kv := range res.Entries {
		...
		case columnPathSpanID:
			...
			sp.attributes[traceql.IntrinsicSpanIDAttribute] = traceql.NewStaticID(util.SpanIDToHexString(kv.Value.ByteArray()))
		case columnPathSpanParentSpanID:
			sp.attributes[traceql.IntrinsicParentIDAttribute] = traceql.NewStaticID(util.SpanIDToHexString(kv.Value.ByteArray()))
...
func (c *traceCollector) KeepGroup(res *parquetquery.IteratorResult) bool {
	...
	for _, e := range res.Entries {
		...
		case columnPathTraceID:
			...
			c.traceAttrs[traceql.IntrinsicTraceIDAttribute] = traceql.NewStaticID(util.TraceIDToHexString(e.Value.ByteArray()))
