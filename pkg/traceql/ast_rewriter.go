...
func (f *fieldExpressionRewriter) RewriteRoot(r *RootExpr) *RootExpr {
	...
	if !r.IsLeaf() {
		return &RootExpr{
			Op:  r.Op,
			LHS: f.RewriteRoot(r.LHS),
			RHS: f.RewriteRoot(r.RHS),
		}
	}

	pipeline, rwCount := f.rewritePipeline(r.Leaf.Pipeline)

	return &RootExpr{
		Leaf: &Expr{
			Pipeline:           pipeline,
			MetricsPipeline:    r.Leaf.MetricsPipeline,
			MetricsSecondStage: r.Leaf.MetricsSecondStage,
			Hints:              r.Leaf.Hints,
			OptimizationCount:  r.Leaf.OptimizationCount + rwCount,
		},
	}
}
